...
\begin{document}
	...
	\begin{frame}{研究背景}
		\begin{itemize}
			...
			\item 广泛应用于工程传热分析、地热模拟、材料热处理等领域。
			\item 应用已从传统传热分析拓展到诸多新兴领域：岩土工程、工程控制。
			\item 既是解决工程算法缺陷的创新工具，又是揭示复杂介质物理机制的理论基础。
			\item 面对复杂几何形状、多变材料属性和复杂边界条件时，解析解难以获得甚至不存在。
		\end{itemize}
		
		\begin{block}{研究定位与核心挑战}
			本文探究利用分片线性有限元方法求解热传导方程的理论分析与数值验证。
		\end{block}
	\end{frame}
	...
	\begin{frame}{Sobolev 空间与有限元空间}
		\begin{defn}[Sobolev 空间 $H^m(\Omega)$]
			\[
			H^m(\Omega) = \{ u \in L^2(\Omega) : D^\alpha u \in L^2(\Omega), |\alpha| \leq m \}.
			\]
			特别地，$H^1_0(\Omega)$ 为边界上为零的 $H^1(\Omega)$ 子空间。
		\end{defn}
		
		\begin{defn}[分片线性有限元空间(P1)]
			设 $\mathcal{T}_h$ 为 $\Omega$ 的剖分，定义有限元空间：
			\[
			V_h = \{ v_h \in C(\bar{\Omega}) : v_h|_K \in \mathrm{P}1(K), \forall K \in \mathcal{T}_h, v_h|_{\partial\Omega} = 0 \}.
			\]
		\end{defn}
	\end{frame}
...
\end{frame}
	...
	\begin{frame}{变分形式}
		...
		a(u,v) = \int_{\Omega} \nabla u \cdot \nabla v \,dx.
		\]
		则变分形式为：求 \( u(t) \in H^1_0(\Omega) \)，使得对任意 \( v \in H^1_0(\Omega) \) 有
		\[
	...
	\begin{frame}{半离散格式——空间离散}
		对空间变量采用分片线性有限元离散。将求解区间 $\Omega = [a,b]$ 剖分为 $N$ 个子区间，节点为
		\[
		a = x_1 < x_2 < \cdots < x_{N+1} = b.
		\]
		\begin{block}{有限元空间 $V_h \subset H_0^1(\Omega)$}
			\[
			V_h = \left\{v_h \in C^0(\overline{\Omega}) : \left. v_h \right|_{[x_{i-1},x_i]} \in \mathrm{P}1, \ i = 2, \ldots, N+1,\ v_h(a) = v_h(b) = 0 \right\}.
			\]
		\end{block}
		其中 $\mathrm{P}1$ 表示一次多项式。
	\end{frame}
	
\begin{frame}{半离散格式——空间离散（基函数）}
   \begin{itemize}
...
\end{frame}
	...
	\begin{frame}{全离散BDF2-P1格式（3/3）}
		...
		令 $u_h^m \approx u_h(x,t_m)$，$f^m = f(x, t_m)$，记时间步长 $\Delta t=\tau$。
		\begin{itemize}
			\item 全离散BDF2-P1有限元格式
			\[
			\begin{cases}
		...
		\end{itemize}
	\end{frame}
	\begin{frame}{全离散BDF2-P1格式的稳定性分析}
		\begin{thm}[全离散 BDF2-P1 格式的稳定性]
			...
			\]
			其中 $\|\cdot\|$ 表示 $L^2(\Omega)$ 范数。特别地，当 $f \equiv 0$ 时，有离散能量 $E_h^m \leq E_h^{m-1}$，即能量随时间非增。
		\end{thm}
	\end{frame}
	
	\begin{frame}{全离散BDF2-P1格式的误差估计}
...
\begin{frame}{一维数值算例 - 模型与理论}
\textbf{例1.} 考虑一维非齐次热传导方程初边值问题：
	\[
	\begin{cases}
...
\begin{frame}{二维情形 - 模型与理论}
	\textbf{例2.}二维热传导方程初边值问题：
	\[
	\begin{cases}
...
\begin{frame}{非线性算例 - 模型与方法}
\textbf{例3.}非线性抛物型方程初边值问题：
...
\end{frame}
	...
	\begin{frame}{非线性算例 - 数值结果与分析}
		\vspace{-0.2cm}
		\begin{columns}[T, onlytextwidth]
			\begin{column}{0.40\textwidth}
				\centering
				\includegraphics[width=\linewidth]{nonlinear.png}
				\footnotesize   图5：BDF2-P1数值解在不同时刻的演化
			\end{column}
